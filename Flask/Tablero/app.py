...
from redmine_api import get_issues_by_update, get_issues_by_create, redmine, author_ids_dict
...
@app.route('/get-author-data', methods=['GET'])
def get_author_data():
    return jsonify({'author_ids_dict': author_ids_dict})
...
def reporter():
    if request.method == 'POST':
        ...
        selected_operators = [int(value) for value in selected_operators]

        print(selected_operators)

        df_updated, df_created = process_issues(issues_updated, issues_created, selected_operators)
